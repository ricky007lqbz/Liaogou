...
/**
 * C_Logger is a wrapper for logging utils
 * But more pretty, simple and powerful
 */
final class C_LoggerPrinter implements C_LPrinter {
     ...
     */
    private C_LSettings settings;

    /**
     * It is used to change the tag
     *
     * @param tag is the given string which will be used in C_Logger
     */
    @Override
    public C_LSettings init(String tag) {
        if (tag == null) {
        ...
        this.tag = tag;
        this.settings = new C_LSettings();
        return settings;
    }

    @Override
    public C_LSettings getSettings() {
        return settings;
    }

    @Override
    public C_LPrinter t(String tag, int methodCount) {
        if (tag != null) {
            localTag.set(tag);
        }
    ...
    private synchronized void log(int logType, String msg, Object... args) {
        if (settings.getLogLevel() == C_LogLevel.NONE) {
            return;
        }
        String tag = getTag();
    ...
    private int getStackOffset(StackTraceElement[] trace) {
        for (int i = MIN_STACK_OFFSET; i < trace.length; i++) {
            ...
            if (!name.equals(C_LoggerPrinter.class.getName()) && !name.equals(C_Logger.class.getName())) {
                return --i;
            }
        }
        return -1;
